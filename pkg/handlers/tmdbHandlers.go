...
func trendingByTypeAndPage(mediaType, page, apiKey string) (*http.Response, error) {
...
func GetGenreNameFromId(mediaType, apiKey string, id int) (string, error) {
...
func GetTrendingLanding(c *config.Repository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var resp responses.Response
		apiKey := c.Config.ApiKey
		mtResp, err := trendingByTypeAndPage("movie", "1", apiKey)
		if err != nil {
			resp.Build(500, err.Error(), nil)
			resp.Respond(w)
			return
		}
		ttResp, err := trendingByTypeAndPage("tv", "1", apiKey)
		if err != nil {
			resp.Build(500, err.Error(), nil)
			resp.Respond(w)
			return
...
func GetTrending(c *config.Repository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
...
func GetMediaWithAllAttributes(c *config.Repository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		mediaType := r.URL.Query().Get("media_type")
...
func GetMediaCategories(c *config.Repository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var resp responses.Response
		apiKey := c.Config.ApiKey
...
func GetCategoryResultsByTypeAndPage(c *config.Repository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		catResp, err := http.Get(TMDB_BASE_URL + "/discover/" +
			...
			genre + "&sort_by=" + sort + "&page=" + page + "&api_key=" + c.Config.ApiKey)
		if err != nil {
			resp.Build(500, err.Error(), nil)
			resp.Respond(w)
			return
		...
		var cats interface{}
		err = json.NewDecoder(catResp.Body).Decode(&cats)
		if err != nil {
			resp.Build(500, "json decoding error", nil)
			resp.Respond(w)
			return
		}
		resp.Build(200, "success", cats)
		resp.Respond(w)
	}
}
func SearchMedia(c *config.Repository) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var resp responses.Response
		var tvResult *http.Response
		var result models.TMDBBaseStruct
		commonParams := "&language=en-US&page=1&include_adult=false&api_key=" + c.Config.ApiKey
